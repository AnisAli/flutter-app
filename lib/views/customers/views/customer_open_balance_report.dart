...
class CustomerOpenBalanceReport
    extends GetView<CustomerOpenBalanceReportController> {
  static const String routeName = '/customerOpenBalanceReport';

  const CustomerOpenBalanceReport({super.key});
    ...
    return Obx(() => controller.isLoading.value
        ...
        : Table(
            ...
            children: [
              ...
              for (var i = 0;
                  i < controller.customerOpenBalanceReports.length;
                  i++)
                TableRow(
                  decoration:
                  ...
                  children: [
                    Padding(
                      ...
                      child: Text(
                        DateFormat('MM-dd-yy').format(DateTime.parse(controller
                                .customerOpenBalanceReports[i].date ??
                            '')),
                    ...
                    Padding(
                      padding: const EdgeInsets.symmetric(vertical: 8.0),
                      child: Text(
                        '${controller.customerOpenBalanceReports[i].transactionNumber}',
                        style: context.captionLarge,
                        overflow: TextOverflow.ellipsis,
                        maxLines: 1,
                      ),
                    ...
                    Padding(
                      padding: const EdgeInsets.symmetric(vertical: 8.0),
                      child: Text(
                        "\$${controller.customerOpenBalanceReports[i].amount}",
                        style: context.captionLarge,
                        overflow: TextOverflow.ellipsis,
                        maxLines: 1,
                      ),
                    ...
                    Padding(
                      padding: const EdgeInsets.symmetric(vertical: 8.0),
                      child: Text(
                        "\$${controller.customerOpenBalanceReports[i].invoiceBalance}",
                        style: context.captionLarge,
                        overflow: TextOverflow.ellipsis,
                        maxLines: 2,
                      ),
  ...
  Widget _buildCompanyInfoDisplay(BuildContext context) {
    return Column(
      ...
      children: [
        Text(
          controller.argCustomer.companyName ?? '',
          style: context.captionMedium.copyWith(fontSize: 14),
        ),
        Text(
          controller.argCustomer.address1 ?? '',
          style: context.captionMedium.copyWith(fontSize: 14),
          maxLines: 3,
        ...
        Text(
          controller.argCustomer.email ?? '',
          style: context.captionMedium.copyWith(fontSize: 14),
        ),
        Text(
          controller.argCustomer.phoneNo ?? '',
          style: context.captionMedium.copyWith(fontSize: 14),
        ),
      ],
    );
  ...
  Widget _buildSimpleAppBar(BuildContext context) {
    return SliverAppBar(
      ...
      centerTitle: true,
      title: Text(
        controller.argCustomer.customerName ?? '',
